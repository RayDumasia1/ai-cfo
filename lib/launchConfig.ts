 ...
 * - billing_period_end passes → account suspended
 * - 30-day grace from billing_period_end to restore
 * - Restore within 30 days → full status back, same member number
 * - After 30 days → permanently lost, no exceptions
 * - Spots do NOT return to pool (hard cap: 50 total issued)
 *
 * This policy is intentional and permanent.
